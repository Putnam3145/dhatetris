...
extern(C) @nogc nothrow {
	...
	void glDeleteLists(uint,uint);
	uint glGenLists(uint);
...
template makeCachedPrimitive(alias func)
{
	...
	void initialize()
	{
		_ourList = glGenLists(1);
		glNewList(_ourList,GL_COMPILE);
		func();
		glEndList();
	}
	static void render()
	...
	alias makeCachedPrimitive = render;
}
...
final class OpenGlTexture {
	...
	void cacheDrawCommand()
	{
		glNewList(_list,GL_COMPILE);
		glBindTexture(GL_TEXTURE_2D, _tex);
		glBegin(GL_QUADS); 
			glTexCoord2f(0, 0); 				glVertex2i(0, 0);
			glTexCoord2f(texCoordWidth, 0); 		glVertex2i(_width, 0); 
			glTexCoord2f(texCoordWidth, texCoordHeight); 	glVertex2i(_width, _height); 
			glTexCoord2f(0, texCoordHeight); 		glVertex2i(0, _height); 
		glEnd();
